...
var HelpDocument = function(program) {
  ...
  if(this.cmd.sections()) {
    var ckeys = Object.keys(this.cmd.sections()), i, k;
...
HelpDocument.prototype.custom = function(data, stream, key) {
  ...
  var text = find(key, this.cmd.sections());
  if(!text) text = '';
...
HelpDocument.prototype.render = function(key, data, stream, section) {
  ...
  if(section) {
    if(typeof section === 'object' && section.text) {
...
HelpDocument.prototype.hasSection = function(key) {
  var sections = Object.keys(this.cmd.sections());
  if(~sections.indexOf(key)) {
    return true;
  }
...
HelpDocument.prototype.getOptionMetrics = function(keys, target) {
  ...
  for(i = 0;i < keys.length;i++) {
    ...
    str = arg.getOptionString(
      this.delimiter, this.assignment, null, !(arg.isCommand()));
...
HelpDocument.prototype.getDescription = function(description, detail) {
  ...
  description = Description.toDescription(description);
...
HelpDocument.prototype.write = function(data, stream) {
  ...
  var i, k, method, args, result, ttyc;

  // disable the ttycolor error stream redirect
  // for help output
  try{
    ttyc = require('ttycolor');
    if(ttyc && ttyc.stderr()) {
      ttyc.stderr(false);
    }
  }catch(e){}
